...
fn build_firewall_lock_args_with_version_flags(version: u8, flags: u8) -> Bytes {
    ...
    // + registry args len(2) + registry args(N) + inner code_hash(32) + inner hash_type(1)
    // + inner args len(2) + inner args(M)
    let mut args = Vec::new();
    args.push(version);
    args.push(flags);

    // * Registry identity (dummy values for tests that fail before use)
    args.extend_from_slice(&[0u8; 32]);
    args.push(0x01);
    args.extend_from_slice(&0u16.to_le_bytes());

    // * Inner lock identity (always_success placeholder)
...
fn build_full_lock_args(
    ...
    let mut args = vec![version, flags];
...
fn test_reject_unsupported_flags() {
    let mut context = Context::default();
    let firewall_out_point = context.deploy_cell(Bytes::from(FIREWALL_BINARY.to_vec()));
    // * bits 2-7 reserved; set bit 2 to trigger UnsupportedFlags
    let lock_args = build_firewall_lock_args_with_version_flags(0x01, 0x04);
    let tx = build_tx_with_firewall_lock(
        &mut context,
        firewall_out_point,
        lock_args,
...
fn test_reject_invalid_registry_data() {
    ...
    let registry_type_script = context
        .build_script(&type_code_out_point, Bytes::from(vec![0x01]))
    ...
    let lock_args = build_full_lock_args(0x01, 0x01, &registry_type_script, inner_hash, 0x01, &[]);
    let tx = build_tx_with_firewall_lock(
        &mut context,
        firewall_out_point,
        lock_args,
...
fn test_reject_registry_not_sorted() {
    ...
    let registry_type_script = context
        .build_script(&type_code_out_point, Bytes::from(vec![0x02]))
        .expect("build registry type script");
...
fn test_reject_blacklisted_output_lock_args() {
    ...
    let registry_type_script = context
        .build_script(&type_code_out_point, Bytes::from(vec![0x03]))
        .expect("build registry type script");
...
fn test_reject_blacklisted_output_type_args() {
    ...
    let registry_type_script = context
        .build_script(&type_code_out_point, Bytes::from(vec![0x04]))
        .expect("build registry type script");
    ...
    let inner_hash: [u8; 32] = context
        ...
        .code_hash()
        .unpack();
    let lock_args = build_full_lock_args(0x01, 0x02, &registry_type_script, inner_hash, 0x01, &[]);
...
fn test_reject_ambiguous_registry_cell_dep() {
    ...
    let registry_type_script = context
        .build_script(&type_code_out_point, Bytes::from(vec![0x05]))
        .expect("build registry type script");
...
fn run_non_blacklisted_pass_and_get_cycles(flags: u8, include_type_script: bool) -> u64 {
    ...
    let registry_type_script = context
        .build_script(&type_code_out_point, Bytes::from(vec![0x06]))
        .expect("build registry type script");
    ...
    let lock_args =
        build_full_lock_args(0x01, flags, &registry_type_script, inner_hash, inner_hash_type, &[]);
...
fn run_non_blacklisted_large_registry_and_get_cycles(
    ...
    let registry_type_script = context
        .build_script(&type_code_out_point, Bytes::from(vec![0x07]))
        .expect("build registry type script");
