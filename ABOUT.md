...
High-level map of this repository (crate roots each have their own `Cargo.toml`; there is no single workspace manifest at the repo root):

```
ckb-transaction-firewall/
├── contracts/
│   ├── firewall-lock/          # Lock script (Rust + ckb-std)
│   ├── blacklist-registry/     # Registry type script + governance path
│   └── governance-lock/        # Governance drill lock (supporting scripts)
├── sdk/
│   ├── typescript/             # Pre-flight SDK (TypeScript)
│   └── rust/                   # Pre-flight SDK (Rust)
├── governance/                 # Proposal schema, voting policy
├── tests/
│   ├── unit/                   # VM tests (ckb-testtool) for contracts
│   └── integration/          # Drills, governance evidence
├── docs/                       # Architecture, lock spec, governance, phase runbooks
├── phase3_artifacts/           # Reproducible build / evidence outputs
├── phase4_artifacts/           # Governance verification milestone evidence
...
**Normative policy** (quorum, voting, multisig execution, emergency temporary adds with `expires_at`, and housekeeping) lives in [`governance/voting.md`](./governance/voting.md) and the overview in [`docs/governance.md`](./docs/governance.md). This document stays descriptive; those files are the single source of truth so thresholds cannot drift in two places.
