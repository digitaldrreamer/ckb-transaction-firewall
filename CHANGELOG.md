...
- Ran `ckb-testtool` integration tests in `tests/unit` (22/22 passing: 15 firewall + 7 blacklist-registry).
...
- Added `ckb-testtool` integration tests:
  - `tests/unit/tests/blacklist_registry_tests.rs` (7 tests passing)
...
- Extended integration coverage:
  - `tests/unit/tests/blacklist_registry_tests.rs` now signs governance payloads with deterministic keys and validates strict signature paths (7 passing tests).
