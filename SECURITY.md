...
| M1 | Medium | Open | `VOTE_THRESHOLD` hardcoded at 3 in `proposals.ts`; on-chain threshold not read for vote approval |
...
| L6 | Low | Open | `header_deps: []` has no explanatory comment in `execute.ts`, `anchor.ts`, `reclaim.ts` |
